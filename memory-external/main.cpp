...
int main() {
    ...
    HWND hWnd = CreateWindowExA(WS_EX_TRANSPARENT | WS_EX_TOPMOST | WS_EX_LAYERED | WS_EX_TOOLWINDOW, " ", "cs2-external-esp", WS_POPUP,
        g::gameBounds.left, g::gameBounds.top, g::gameBounds.right - g::gameBounds.left, g::gameBounds.bottom + g::gameBounds.left, NULL, NULL, hInstance, NULL);
    ...
    while (GetMessage(&msg, NULL, 0, 0) && !finish)
    {
        ...
        if (GetAsyncKeyState(VK_INSERT) & 0x8000)
        {
            if (!show_gui)
            {
                ...
                show_gui = true;
                std::this_thread::sleep_for(std::chrono::milliseconds(500)); // Wait for GUI
                if (gui_has_focus) // Check if GUI window exists
                {
                    gui_has_focus = true;
                }
            }
