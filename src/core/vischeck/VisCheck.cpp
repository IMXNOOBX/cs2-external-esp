...
namespace {
    ...
    bool LoadTriFile(const std::string& path, std::vector<TriangleCombined>& out) {
        ...
        if (size <= 0 || static_cast<long long>(size) % sizeof(TriangleCombined) != 0)
            return false;
...
bool VisCheck::IntersectBVHDetailed(const BVHNode* node, const Vector3& origin, const Vector3& dir, float maxDist, float& hitDist, Vector3& hitNormal) const {
    if (!node || !node->bounds.RayIntersects(origin, dir))
        return false;
    ...
    if (node->IsLeaf()) {
        for (const auto& tri : node->triangles) {
            float t;
            Vector3 n;
            if (RayIntersectsTriangleDetailed(origin, dir, tri, t, n) && t < maxDist && t < hitDist) {
                hitDist = t;
                hitNormal = n;
                hit = true;
            }
    ...
    if (node->left)
        hit |= IntersectBVHDetailed(node->left.get(), origin, dir, maxDist, hitDist, hitNormal);
    if (node->right)
        hit |= IntersectBVHDetailed(node->right.get(), origin, dir, maxDist, hitDist, hitNormal);
    return hit;
}
...
VisCheck::HitResult VisCheck::RayCast(const Vector3& origin, const Vector3& dir, float maxDist) const {
    HitResult result;
    result.hit = false;
    ...
    Vector3 normal;
    for (const auto& root : bvhNodes) {
        IntersectBVHDetailed(root.get(), origin, dir, maxDist, result.distance, normal);
    }
...
bool VisCheck::RayIntersectsTriangleDetailed(const Vector3& origin, const Vector3& dir, const TriangleCombined& tri, float& t, Vector3& normal) {
